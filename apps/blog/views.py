...
class ListPostByCategoryView(APIView):
    ...
    def get(self, request ,format=None):
        if Post.postobjects.all().exists():
            ...
            # #Si categoria tiene padre, solo se filtra con esa y no con el padre
            # if category.parent:
            #     posts = posts.filter(category=category)
            # else: #si no tiene categoria padre
            #     #filtra solo la categoria
            if not Category.objects.filter(parent=category).exists():
                posts = posts.filter(category=category)
                #si esta categoria padre tiene hijos, filtrar por ambos
            else:
